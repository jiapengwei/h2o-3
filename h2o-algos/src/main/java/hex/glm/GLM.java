      ...
      try {
        while (true) {
          ...
          if(_state._lsNeeded) {
            ...
            if (betaCnd.length != oldBetaCnd.length) {  // ln 1453 is skipped and betaCnd.length != _state.beta()
    ...
    protected Submodel computeSubmodel(int i,double lambda, double nullDevTrain, double nullDevValid) {
      ...
      if(lambda >= _lmax && _state.l1pen() > 0)
        ...
        do {
          ...
          //debugging for Wendy
          int stateActiveCols = _state.activeData()._activeCols==null?_state.activeData().fullN()+1:_state.activeData()._activeCols.length;
          Log.info(LogMsg("\n ##### fold number "+_parms._cv_fold+" _state.activeCols number "+stateActiveCols));
          fitModel();
